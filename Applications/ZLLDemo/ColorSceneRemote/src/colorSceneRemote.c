/**************************************************************************//**
  ...
  \internal
    ...
    18/02/15 Yogesh Bellan     - modified for Touch Integration
******************************************************************************/
...
|| defined(AT90USB1287) || defined(ATMEGA128RFA1) || defined(ATMEGA256RFR2) || defined(ATMEGA2564RFR2)
...
static void init(void)
{
  ...
  N_PacketDistributor_RegisterEndpoint(&appEndpoint, 0u);
...
static void appJoyStickInd(BSP_JoystickState_t state)
{
  ...
  if(colorSet)
  {
    ...
    cmd->payload.moveToColor.colorX = X; /* BLUE */
  ...
  else if((SLIDER == button) && (event == TOUCHKEY_PRESSED_EVENT))
  {
    ...
    if(sliderPosition != sliderInitialState)
    {
      ...
      if(triggerPacket >= 1)
      {
        if (!(cmd = clustersAllocCommand()))
          return;
        cmd->payload.stepLevel.stepSize = 40;
        if(lightMov)
