...
In dieser Woche habe ich zum einem mit den Pflichtenheft befasst und welche Sensoren wir haben was diese können und die passenden Datenblätter~\cite{datasheetcss811}~\cite{datasheetscd41}~\cite{datasheetsht21} zusammen gesucht und diese durch gegangen um die Sensoren genauer zu verstehe und deren Funktionsweise. Danach habe ich mich mit dem Funkmodulen auseinander gesetzt und wollte den Temperatur Sensor aus dem ZigBee-Set(LM73) und das ZigBee Netzwerk mit einander verbinden, so das die Sensor Daten an den Koordinator weiter geleiten werden. In dieser Zeit habe ich mich erneut mit dem I$^2$C-Bus und der Übertragung von Daten beschäftigt.
