<template>
  ...
  <div id="tvoc-div">
    <tvoc-chart
        v-if="loaded"
        v-bind:chart-data1="tvocData1"
        v-bind:chart-data2="tvocData2"
        v-bind:chart-data3="tvocData3"
        v-bind:x-axis="labelsTvoc"
    />
    <button class="button is-success" v-bind:disabled="waitButton" @click="sixHours('tvoc')">6 Stunden</button>
    <button class="button is-success" v-bind:disabled="waitButton" @click="oneDay('tvoc')">1 Tag</button>
    <button class="button is-success" v-bind:disabled="waitButton" @click="oneWeek('tvoc')">1 Woche</button>
  </div>
</template>
      ...
      methods: {
        ...
        axisCalculation(medianSize, options) {
          ...
          // ##### HUMIDITY #####
            ...
            let finalSensorData3 = []
            for (let i = 0; i < 120; i++) {
              this.humData1.push(null)
            }
            for (let i = 0; i < 120; i++) {
              this.humData2.push(null)
            }
            for (let i = 0; i < 120; i++) {
              this.humData3.push(null)
            }
            for (let i = 0; i < this.sensorData1.length; i++) {
              if (i % medianCalculationSize === 0 && i !== 0) {
                if (this.sensorData1[i])
                  finalSensorData1.push({
                    data: parseFloat(this.sensorData1[i].humSCD41) / 100,
                    timestamp: this.sensorData1[i].created_at
                  })
              }
            }
            for (let i = 0; i < this.sensorData2.length; i++) {
              if (i % medianCalculationSize === 0 && i !== 0) {
                if (this.sensorData2[i])
                  finalSensorData2.push({
                    data: parseFloat(this.sensorData2[i].humSCD41) / 100,
                    timestamp: this.sensorData2[i].created_at
                  })
              }
            }
            for (let i = 0; i < this.sensorData3.length; i++) {
              if (i % medianCalculationSize === 0 && i !== 0) {
                if (this.sensorData3[i])
                  finalSensorData3.push({
                    data: parseFloat(this.sensorData3[i].humSCD41) / 100,
                    timestamp: this.sensorData3[i].created_at
                  })
              }
            }
            ...
            finalSensorData1.forEach(data=>{
              if(nowDate.isAfter(data.timestamp)){
                let diff = moment(data.timestamp,'YYYY-MM-DD HH:mm:ss').diff(nowDate, 'minutes')
                let diffTicks = Math.floor(Math.abs(diff/medianSize))
                this.humData1[this.humData1.length-diffTicks] = data.data
                ...
                switch (medianSize){
                  ...
                  case 3: {
                    if(diffTicks === 0)
                      diffTicks = 1
                    this.labelsHum[this.humData1.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('HH:mm')
                    break;
                  }
                  case 84: {
                    if(diffTicks === 0)
                      diffTicks = 1
                    this.labelsHum[this.humData1.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('DD dd:HH:mm')
                    break;
                  }
                }
              }
            ...
            finalSensorData2.forEach(data=>{
              if(nowDate.isAfter(data.timestamp)){
                let diff = moment(data.timestamp,'YYYY-MM-DD HH:mm:ss').diff(nowDate, 'minutes')
                let diffTicks = Math.floor(Math.abs(diff/medianSize))
                this.humData2[this.humData2.length-diffTicks] = data.data
                switch (medianSize){
                  case 12:
                  case 3: {
                    this.labelsHum[this.humData2.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('HH:mm')
                    break;
                  }
                  case 84: {
                    this.labelsHum[this.humData2.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('DD dd:HH:mm')
                    break;
                  }
                }
              }
            ...
            finalSensorData3.forEach(data=>{
              if(nowDate.isAfter(data.timestamp)){
                let diff = moment(data.timestamp,'YYYY-MM-DD HH:mm:ss').diff(nowDate, 'minutes')
                let diffTicks = Math.floor(Math.abs(diff/medianSize))
                this.humData3[this.humData3.length-diffTicks] = data.data
                switch (medianSize){
                  case 12:
                  case 3: {
                    this.labelsHum[this.humData3.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('HH:mm')
                    break;
                  }
                  case 84: {
                    this.labelsHum[this.humData3.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('DD dd:HH:mm')
                    break;
                  }
                }
              }
          ...
          // ##### CO2 #####
            ...
            let finalSensorData3 = []
            for (let i = 0; i < 120; i++) {
              this.tvocData1.push(null)
            }
            for (let i = 0; i < 120; i++) {
              this.tvocData2.push(null)
            }
            for (let i = 0; i < 120; i++) {
              this.tvocData3.push(null)
            }
            for (let i = 0; i < this.sensorData1.length; i++) {
              if (i % medianCalculationSize === 0 && i !== 0) {
                if (this.sensorData1[i])
            ...
            finalSensorData1.forEach(data=>{
              if(nowDate.isAfter(data.timestamp)){
                let diff = moment(data.timestamp,'YYYY-MM-DD HH:mm:ss').diff(nowDate, 'minutes')
                let diffTicks = Math.floor(Math.abs(diff/medianSize))
                this.tvocData1[this.tvocData1.length-diffTicks] = data.data
                console.log(medianSize)
                switch (medianSize){
                  case 12:
                  case 3: {
                    if(diffTicks === 0)
                      diffTicks = 1
                    this.labelsTvoc[this.tvocData1.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('HH:mm')
                    break;
                  }
                  case 84: {
                    if(diffTicks === 0)
                      diffTicks = 1
                    this.labelsTvoc[this.tvocData1.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('DD dd:HH:mm')
                    break;
                  }
                }
              }
            ...
            finalSensorData2.forEach(data=>{
              if(nowDate.isAfter(data.timestamp)){
                let diff = moment(data.timestamp,'YYYY-MM-DD HH:mm:ss').diff(nowDate, 'minutes')
                let diffTicks = Math.floor(Math.abs(diff/medianSize))
                this.tvocData2[this.tvocData2.length-diffTicks] = data.data
                switch (medianSize){
                  case 12:
                  case 3: {
                    this.labelsTvoc[this.tvocData2.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('HH:mm')
                    break;
                  }
                  case 84: {
                    this.labelsTvoc[this.tvocData2.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('DD dd:HH:mm')
                    break;
                  }
                }
              }
            ...
            finalSensorData3.forEach(data=>{
              if(nowDate.isAfter(data.timestamp)){
                let diff = moment(data.timestamp,'YYYY-MM-DD HH:mm:ss').diff(nowDate, 'minutes')
                let diffTicks = Math.floor(Math.abs(diff/medianSize))
                this.tvocData3[this.tvocData3.length-diffTicks] = data.data
                switch (medianSize){
                  case 12:
                  case 3: {
                    this.labelsTvoc[this.tvocData3.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('HH:mm')
                    break;
                  }
                  case 84: {
                    this.labelsTvoc[this.tvocData3.length - diffTicks] = moment(data.timestamp, 'YYYY-MM-DD HH:mm:ss').format('DD dd:HH:mm')
                    break;
                  }
                }
              }
            ...
            switch (hours) {
              case 6: {
                ...
                for (let i = 0; i < 120; i++) {
                  const date = moment(startDate).add(3, 'minutes').format('HH:mm');
                  startDate.setMinutes(startDate.getMinutes() + 3)
                  this.labelsTvoc.push(date.toString());
                }
                break;
              }
              case 24: {
