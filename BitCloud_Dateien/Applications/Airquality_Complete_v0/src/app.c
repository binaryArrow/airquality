/***************************************************************************
  ...
  \author Markus Krauße, Achim Glaesmann, Lucas Merkert, Friedrich
  ******************************************************************************/
  /*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
  /*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
...
void APL_TaskHandler(void){
	switch(appstate){
		...
		int32_to_str(tmp, sizeof(tmp),SHT_tmp_vorkomma, 5, 3);
		int32_to_str(tmp, sizeof(tmp),SHT_tmp_nachkomma, 8, 2);
		int32_to_str(tmp, sizeof(tmp),SHT_rh_vorkomma, 11, 3);
		int32_to_str(tmp, sizeof(tmp),SHT_rh_nachkomma, 14, 2);
