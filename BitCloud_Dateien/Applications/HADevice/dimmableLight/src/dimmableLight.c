...
static uint8_t dlClustersBoundMask;
/******************************************************************************
                    Prototypes section
******************************************************************************/
...
static void handleDlConfigReporting(void)
{
  ...
  switch (configureReportingState)
  {
    case CONFIG_REPORT_INIT_STATE:
      if (dlClustersBoundMask & ON_OFF_CLUSTER_BOUND)
      {
        ...
        configureReportingState = ON_OFF_REPORTING_CONFIGURED;
        dlClustersBoundMask &= ~ON_OFF_CLUSTER_BOUND;
  	    break;
      }
    case ON_OFF_REPORTING_CONFIGURED:
      if (dlClustersBoundMask & LEVEL_CONTROL_CLUSTER_BOUND)
      {
        sendConfigureReportingToNotify(APP_SRC_ENDPOINT_ID, APP_ENDPOINT_COMBINED_INTERFACE, 
        ...
        dlClustersBoundMask &= ~LEVEL_CONTROL_CLUSTER_BOUND;
	    break;
      }
...
{
  if (bindInd->clusterId == LEVEL_CONTROL_CLUSTER_ID)
    dlClustersBoundMask |= LEVEL_CONTROL_CLUSTER_BOUND;
  else if(bindInd->clusterId == ONOFF_CLUSTER_ID)
    dlClustersBoundMask |= ON_OFF_CLUSTER_BOUND;
}
