"""
@testcase
@description 4.1 TP-NWI-TC-01: Classical joining DUT ZR to ZLL network
...
dutZR2 = deviceScannerGetAssociatedPort(TEST_DEVICE_TYPE_ROUTER, portList)
...
# Test procedure
#*****************************************************************************************\
writeLog("Item 1: Power on DUT ZR2")
powerOn([dutZR2])
resetRouterToFN([dutZR2])

writeLog("Item 2a: By application specific means, DUT ZR2 performs a classical network \
          scan on the primary channels but finds no networks with associate permit set to TRUE")
sleep(1)
writeLog("Item 2b: DUT ZR2 then performs a classical network scan on the secondary \
          channels but finds no networks with associate permit set to TRUE")
sleep(1)
writeLog("Item 2c: DUT ZR2 may repeat the above scans or simply wait for further instructions")
sleep(1)
...
writeLog("Item 4a: By application specific means, DUT ZR2 performs a classical network scan \
         on the primary channels and finds the ZLL network with associate permit set to TRUE")
sleep(1)
writeLog("Item 4b: DUT ZR2 attempts to associate to the network through ZR1")
sleep(1)
writeLog("Item 4c: ZR1 confirms the association request and allocates a random address to DUT ZR2")
dutZR2NwkAddr = nwkAssociation(dutZR2, BC_SUCCESS_STATUS, BC_SUCCESS_STATUS)
check(dutZR2NwkAddr != None)

writeLog("Item 5: ZR1 transports the network key to DUT ZR2, encrypted with the \
         certification pre-installed link key")
sleep(1)

writeLog("Item 6: DUT ZR2 announces itself on the network")
idle([ZED1, ZR1, dutZR2])
