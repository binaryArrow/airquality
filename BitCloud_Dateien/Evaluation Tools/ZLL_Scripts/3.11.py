"""
@testcase
@description 3.11 TP-CST-TC-11: Groups cluster with server as DUT
...
sendAddGroupCommand(client1, broadcastAllAddr, broadcastEndpoint, group3Id)
check(receiveAddGroupInd(server1) == group3Id)
idle([client1, server1], 1000)

writeLog("5 Client sends view group #3 command")
sendViewGroupCommand(client1, group3Id)
check(receiveViewGroupInd(server1) == group3Id)
receiveViewGroupResp(client1, 0, group3Id)
...
sendGetGroupMembershipCommand(client1, [group3Id])
receiveAndCheck(server1, getGroupMembershipIndStr)
...
sendGetGroupMembershipCommand(client1, [])
receiveAndCheck(server1, getGroupMembershipIndStr)
receiveGetGroupMembershipResp(client1, 3, [group1Id, group2Id, group3Id])
...
sendAddGroupIfIdentifyingCommand(client1, group1Id)
check(receiveAddGroupIfIdentifyingInd(server1) == group1Id)
