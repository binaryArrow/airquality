"""
@testcase
@description 4.12 TP-NWI-TC-12: Touchlinking NFN router to a ZLL end device on a non ZLL network
...
writeLog("Item P10: Power on ZED2 and ZR1. (Reset devices to FN)")
resetRouterToFN([ZR1])
resetEndDeviceToFN([ZED2])

writeLog("Item P11: User initiates a touchlink command on ZED2 with ZED2 held close to ZR1. Touchlink should succeed")
touchlink(ZED2, ZR1)
idle([ZED2, ZR1])
ZR1NwkAddr = getNwkAddress(ZR1)
writeLog("ZR1 network address - 0x%04X" % ZR1NwkAddr)

writeLog("Item P12: User initiates a turn on command on ZED2")
setAddressing(ZED2, APS_SHORT_ADDRESS, ZR1NwkAddr, APP_ENDPOINT_LIGHT)
sendOnCommand(ZED2)
receiveOnCommand(ZR1)

writeLog("Item P13: User initiates a turn off command on ZED2")
sendOffCommand(ZED2)
receiveOffCommand(ZR1)

idle([ZC, dutZED1, ZR1, ZED2])
...
writeLog("Item 3: DUT ZED1 does not transmit a network start request inter-PAN command frame \
