"""
@testcase
@description 4.9 TP-NWI-TC-09: Classical joining non-ZLL ZR to DUT ZR on ZLL network
...
ZR2 = deviceScannerGetAssociatedPort(TEST_DEVICE_TYPE_NON_ZLL_ROUTER, portList)
...
haResetToFN([ZR2], BC_FAILURE_STATUS)
sleep(5)

writeLog("Item 2a: By application specific means, ZR2 performs a classical network scan on \
         all channels but finds no networks with associate permit set to TRUE.")
haRestartNwk(ZR2, channel, BC_FAILURE_STATUS)
writeLog("Item 2b: ZR2 may repeat the above scans or simply wait for further instructions.")
sleep(1)
...
writeLog("Item 4a: By application specific means, ZR2 performs a classical network scan on all \
...
writeLog("Item 6: If the transport key command frame can be decrypted, ZR2 announces itself on \
