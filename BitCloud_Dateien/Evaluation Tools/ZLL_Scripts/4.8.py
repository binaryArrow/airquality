"""
@testcase
@description 4.8 TP-NWI-TC-08: Classical joining ZED to DUT ZR on ZLL network
...
ZED2 = deviceScannerGetAssociatedPort(TEST_DEVICE_TYPE_END_DEVICE, portList)
...
powerOn([ZED2])
resetEndDeviceToFN([ZED2])

writeLog("Item 2a: By application specific means, ZED2 performs a classical network \
          scan on the primary channels but finds no networks with associate permit set to TRUE")
nwkAssociation(ZED2, BC_FAILURE_STATUS, BC_FAILURE_STATUS)
writeLog("Item 2b: ZED2 then performs a classical network scan on the secondary \
          channels but finds no networks with associate permit set to TRUE")
sleep(1)
writeLog("Item 2c: ZED2 may repeat the above scans or simply wait for further instructions")
sleep(1)
...
writeLog("Item 4a: By application specific means, ZED2 performs a classical network scan \
...
dutZED2NwkAddr = nwkAssociation(ZED2, BC_SUCCESS_STATUS, BC_SUCCESS_STATUS)
check(dutZED2NwkAddr != None)

writeLog("Item 5: DUT ZR1 transports the network key to ZED2, setting the source address field \
         to 0xffffffffffffffff and encrypted with the certification pre-installed link key")
sleep(1)

writeLog("Item 6: ZED2 announces itself on the network")
